...
class ZillowScrapper():
    ...
    def run(self):
        url = 'https://www.zillow.com/northbrook-il/sold/'
        #url = 'https://www.zillow.com/northbrook-il/sold/house_type/?searchQueryState=%7B%22pagination%22%3A%7B%7D%2C%22mapBounds%22%3A%7B%22west%22%3A-87.95479335009766%2C%22east%22%3A-87.71790064990235%2C%22south%22%3A42.08605608186619%2C%22north%22%3A42.153540345934054%7D%2C%22mapZoom%22%3A12%2C%22regionSelection%22%3A%5B%7B%22regionId%22%3A33178%2C%22regionType%22%3A6%7D%5D%2C%22isMapVisible%22%3Atrue%2C%22filterState%22%3A%7B%22pmf%22%3A%7B%22value%22%3Afalse%7D%2C%22fore%22%3A%7B%22value%22%3Afalse%7D%2C%22ah%22%3A%7B%22value%22%3Atrue%7D%2C%22sort%22%3A%7B%22value%22%3A%22globalrelevanceex%22%7D%2C%22auc%22%3A%7B%22value%22%3Afalse%7D%2C%22nc%22%3A%7B%22value%22%3Afalse%7D%2C%22rs%22%3A%7B%22value%22%3Atrue%7D%2C%22fsbo%22%3A%7B%22value%22%3Afalse%7D%2C%22cmsn%22%3A%7B%22value%22%3Afalse%7D%2C%22pf%22%3A%7B%22value%22%3Afalse%7D%2C%22fsba%22%3A%7B%22value%22%3Afalse%7D%2C%22manu%22%3A%7B%22value%22%3Afalse%7D%2C%22con%22%3A%7B%22value%22%3Afalse%7D%2C%22mf%22%3A%7B%22value%22%3Afalse%7D%2C%22apa%22%3A%7B%22value%22%3Afalse%7D%2C%22land%22%3A%7B%22value%22%3Afalse%7D%2C%22tow%22%3A%7B%22value%22%3Afalse%7D%7D%2C%22isListVisible%22%3Atrue%7D'

        for page in range(1, 21):
            params = {
                'searchQueryState': '{"pagination":{"currentPage": %s},"mapBounds":{"west":-87.95479335009766,"east":-87.71790064990235,"south":42.08605608186619,"north":42.153540345934054},"mapZoom":12,"regionSelection":[{"regionId":33178,"regionType":6}],"isMapVisible":true,"filterState":{"isPreMarketForeclosure":{"value":false},"isForSaleForeclosure":{"value":false},"isAllHomes":{"value":true},"sortSelection":{"value":"globalrelevanceex"},"isAuction":{"value":false},"isNewConstruction":{"value":false},"isRecentlySold":{"value":true},"isForSaleByOwner":{"value":false},"isComingSoon":{"value":false},"isPreMarketPreForeclosure":{"value":false},"isForSaleByAgent":{"value":false},"isManufactured":{"value":false},"isCondo":{"value":false},"isMultiFamily":{"value":false},"isApartment":{"value":false},"isLotLand":{"value":false},"isTownhouse":{"value":false}},"isListVisible":true}' % page
            }
    ...
    def parse(self, reponse):
        ...
        for card in deck.contents:
            ...
            price = str(card.find('div', {'class': 'list-card-price'}))
            if script and price:
                ...
                self.results.append({
                    ...
                    'price_per_sq_ft': (int(card.find('div', {'class': 'list-card-price'}).text.replace('$', '').replace(',', '').replace('--', '0').replace('Est. ', '').replace('M', '0000').replace('.', ''))/int(str(script_json['floorSize']['value']).replace(',', '').replace('--', '0').replace('None', '1'))),
    ...
    def to_csv(self):
        with open('zillow_sold.csv', 'w') as csv_file:
